...
///
/// Rules:
/// 1. Strip all punctuation except specified punctuation chars and apostrophes in contractions/possessives
/// 2. Split on whitespace, with specified punctuation as separate tokens
...
pub fn tokenize(line: &str, punctuation: &[char]) -> Vec<String> {
    ...
    for c in normalized_line.chars() {
        if c.is_ascii_alphabetic() {
            // Letters can be part of words
            current_token.push(c);  // Keep original case
        } else if c == '\'' {
            // Apostrophes are included in the token
            current_token.push('\'');
        } else if punctuation.contains(&c) {
            // Punctuation chars end current token and become their own tokens
...
mod tests {
    ...
    fn test_tokenize_empty_and_edge_cases() {
        let punct = &[',', '.'];
        assert_eq!(tokenize("", punct), Vec::<String>::new());
        assert_eq!(tokenize("   ", punct), Vec::<String>::new());
        assert_eq!(tokenize("''", punct), Vec::<String>::new());
        assert_eq!(tokenize("'", punct), Vec::<String>::new());
        assert_eq!(tokenize(" ' ", punct), Vec::<String>::new());
        assert_eq!(tokenize("a'", punct), vec!["a"]);  // Trailing quote apostrophe stripped
        assert_eq!(tokenize("'a", punct), vec!["a"]);  // Leading quote apostrophe stripped
        assert_eq!(tokenize("a'b", punct), vec!["a'b"]);  // Internal apostrophe kept
        assert_eq!(tokenize(" leading space", punct), vec!["leading", "space"]);
        assert_eq!(tokenize("trailing space ", punct), vec!["trailing", "space"]);
        assert_eq!(tokenize("token1 token2", punct), vec!["token", "token"]); // Numbers filtered
    }
    ...
    #[test]
    fn test_custom_punctuation_empty() {
    ...
    #[test]
    fn test_custom_punctuation_question_exclamation() {
        let line = "Hello! How are you? I'm fine.";
        let tokens = tokenize(line, &['!', '?']);
        assert_eq!(tokens, vec!["Hello", "!", "How", "are", "you", "?", "I'm", "fine"]);
    }

    #[test]
    fn test_custom_punctuation_semicolon_colon() {
        let line = "First: second; third: fourth.";
        let tokens = tokenize(line, &[':', ';']);
        assert_eq!(tokens, vec!["First", ":", "second", ";", "third", ":", "fourth"]);
    }

    #[test]
    fn test_custom_punctuation_all() {
        let line = "Hello, world! How: are; you?";
        let tokens = tokenize(line, &[',', '!', ':', ';', '?']);
        assert_eq!(tokens, vec!["Hello", ",", "world", "!", "How", ":", "are", ";", "you", "?"]);
    }
    ...
    #[test]
    fn test_custom_punctuation_single_char() {
